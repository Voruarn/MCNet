...
from network.MCNetTR import MCNet, CLIPTextEncoder
...
parser.add_argument("--base_data_path", type=str, 
        ...
        help="所有数据集的根目录")
parser.add_argument('--model', type=str, default='MCNet', 
                    help='模型名称')
...
def validate(model, text_encoder, loader, device, metrics):
    """验证单个数据集，返回所有指标结果"""
    metrics.reset()
    with torch.no_grad():
...
def test_on_dataset(model, text_encoder, dataset_name, data_path, opt, metrics, log_file):
    ...
    # 执行测试
    model.eval()
    results = validate(model, text_encoder, test_loader, device, metrics)
...
if __name__=='__main__':
    ...
    ckpt_name = opt.load.split('/')[-1].split('.')[0]
    log_file = os.path.join(opt.log_dir, f"{ckpt_name}_{timestamp}.txt")
    ...
    for dataset_name, subpath in DATASETS.items():
        ...
        result = test_on_dataset(model, text_encoder, dataset_name, dataset_full_path, opt, metrics, log_file)
        if result:
